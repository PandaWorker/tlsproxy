...
func NewUTLSHTTPRoundTripper(clientHelloID utls.ClientHelloID, uTlsConfig *utls.Config,
	...
	return NewUTLSHTTPRoundTripperWithProxy(clientHelloID, uTlsConfig, backdropTransport, removeSNI, nil, nil)
}
...
func NewUTLSHTTPRoundTripperWithProxy(clientHelloID utls.ClientHelloID, uTlsConfig *utls.Config,
	backdropTransport http.RoundTripper, removeSNI bool, clientHelloSpec *utls.ClientHelloSpec, proxy *url.URL) http.RoundTripper {
	rtImpl := &uTLSHTTPRoundTripperImpl{
		clientHelloID:     clientHelloID,
		clienHelloSpec:    clientHelloSpec,
		config:            uTlsConfig,
...
type uTLSHTTPRoundTripperImpl struct {
	...
	clienHelloSpec *utls.ClientHelloSpec
	config         *utls.Config
...
func (r *uTLSHTTPRoundTripperImpl) dialTLS(ctx context.Context, addr string) (*utls.UConn, error) {
	...
	if r.clientHelloID == utls.HelloCustom {
		if err := uconn.ApplyPreset(r.clienHelloSpec); err != nil {
			return nil, err
		}
	}
